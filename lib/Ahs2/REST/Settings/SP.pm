package Ahs2::REST::Settings::SP;
use strict;
use Ahs2::REST::Settings::Backend;
use Ahs2::REST::Settings::Map;
...
sub service_object {
    ...
    my $backend = Ahs2::REST::Settings::Backend->new(
        {
            model     => $env->{model},
    ...
    my $rest = Ahs2::REST::Engine->new(
        {
            map       => Ahs2::REST::Settings::Map->get_map,
            backend   => $backend,
            view      => $view,
            session   => $env->{session},
            model     => $env->{model},
