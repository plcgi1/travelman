package Ahs2::Component::Upload::Engine;
use strict;
use Data::Dumper;
use JSON::XS;
...
sub validate {
    ...
    if ( $self->request->method eq 'POST' ){
        ...
        # допустимый размер    
        if ( $self->request->uploads->{file}->size > $config->{media}->{user_image}->{maxsize} ) {
            $res = WOA::Validator::ErrorCode->new();
            $res->errorFields([
                { name => 'file', error => $fmt->encode_utf('Недопустимый размер файла') }
            ]);
        }
        # допустимый типа файла
        my $file_type = $self->request->uploads->{file}->content_type;
        my $accept;
        ...
        unless ( $accept ) {
            $res = WOA::Validator::ErrorCode->new();
            $res->errorFields([
                { name => 'file', error => $fmt->encode_utf('Недопустимый формат файла') }
            ]);
        }
    }
    return $res;
